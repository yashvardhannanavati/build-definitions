...
| `GITHUB_REF` | string | No | `""` | Git reference (branch or tag) for the scan results |
...
*Note: `GITHUB_SHA` is mandatory. `GITHUB_REF` is optional and can be provided alongside `GITHUB_SHA`.
...
### Using with Git Reference (Branch/Tag)

```yaml
apiVersion: tekton.dev/v1
kind: TaskRun
metadata:
  name: upload-sarif-with-ref
spec:
  taskRef:
    name: github-sarif-upload
  params:
...
### Using with Security Scanning Tasks

This task is designed to work with security scanning tasks that attach SARIF artifacts to container images:

```yaml
apiVersion: tekton.dev/v1
kind: TaskRun
metadata:
  name: upload-sarif-from-scan
spec:
  taskRef:
    name: github-sarif-upload
  params:
    ...
    - name: GITHUB_SHA
      value: "abc123def456"
    - name: image-url
      value: "quay.io/redhat-user-workloads/rhosdt-tenant/jaeger/jaeger-bundle"
    - name: image-digest
      value: "sha256:a60fcbaa34a5d309091165175e357054a062eb4307482ac132e5e39371136371"
```
...
- Invalid parameter combinations (both GITHUB_REF and GITHUB_SHA provided)
- SARIF artifact discovery failures
- GitHub API errors
- Network connectivity issues
- Retry logic for both ORAS and CodeQL operations

## Security Considerations

- **CodeQL CLI**: Uses GitHub's official CodeQL CLI for secure SARIF uploads
- **Token masking**: GitHub tokens are masked in logs to prevent leakage
- **Secure authentication**: CodeQL CLI handles authentication securely
- **Konflux-test image**: Uses the official `quay.io/konflux-ci/konflux-test` image
- **Kubernetes secrets**: GitHub tokens are stored as Kubernetes secrets
- **Least-privilege access**: Use tokens with minimal required permissions
- **Token rotation**: Monitor token usage and rotate regularly
- **SARIF content**: Ensure SARIF files don't contain sensitive information
- **ORAS security**: Uses ORAS for secure artifact discovery and download
